...
Clustering is an unsupervised ML technique that can be used to study the data and extract patterns. It works with a vectorized representation of the data, and in our case, we will use the word embeddings computed with the **Word2Vec** algorithm available in the `gensim` library.
...
Indeed, the first significant improvement is obtained when switching to DL models, that can generate embeddings of a much higher quality and that capture the semantic meaning of the whole sentence. Finally and as expected, fine-tuned DL models vastly outperform any other model. In particular, the considered cross-encoder reaches state-of-the-art performance on the semantic textual similarity task using the STS dataset.
...
We need to provide a prompt to GPT 3.5, which is a sentence that will be used by the model to generate a response. The prompt we used is the following:
    ...
    Your task is to compute the similarity of multiple pairs of sentences.
    ...
    - Score 0.0: the two sentences are completely dissimilar.
    ```
    ...
    Your answer must contain a list of scores that has one element for each pair received. The first element of the list must be the similarity score of the first pair of sentences, the second element must be the similarity score of the second pair of sentences, and so on and so forth.
...
The pre-trained model we are going to use is `bert-base-uncased`, a BERT model trained on a large corpus of English data, that, among many other tasks, can be used for efficient text classification. It is available in the `transformers` library.
